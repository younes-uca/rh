...
export class ReclamationListAdminComponent extends AbstractListController<ReclamationDto, ReclamationCriteria, ReclamationService>  implements OnInit {
    ...
    destinataires :Array<EmployeDto>;
  
    constructor(reclamationService: ReclamationService, private typeReclamationService: TypeReclamationService, private etatReclamationService: EtatReclamationService, private employeService: EmployeService, private employeService: EmployeService) {
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'destinataire?.id', header: 'Destinataire'},
        ];
    }
    ...
    public async loadDestinataire(){
        await this.roleService.findAll();
        const isPermistted = await this.roleService.isPermitted('Reclamation', 'list');
        isPermistted ? this.employeService.findAll().subscribe(destinataires => this.destinataires = destinataires,error=>console.log(error))
        : this.messageService.add({severity: 'error', summary: 'Erreur', detail: 'Problème de permission'});
    }
   ...
   public prepareColumnExport() : void {
        this.exportData = this.items.map(e => {
            return {
                ...
                'Destinataire': e.destinataire?.id ,
            }
        });
        ...
        //'Destinataire': this.criteria.destinataire?.id ? this.criteria.destinataire?.id : environment.emptyForExport ,
        }];
      }
}
