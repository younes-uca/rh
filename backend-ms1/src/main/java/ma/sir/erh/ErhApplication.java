...
import ma.sir.erh.bean.core.*;
import ma.sir.erh.service.facade.admin.*;

import ma.sir.erh.zynerator.security.common.AuthoritiesConstants;
import ma.sir.erh.zynerator.security.bean.User;
import ma.sir.erh.zynerator.security.bean.Permission;
import ma.sir.erh.zynerator.security.bean.Role;
import ma.sir.erh.zynerator.security.service.facade.UserService;
import ma.sir.erh.zynerator.security.service.facade.RoleService;
...
@EnableCaching
//@EnableFeignClients("ma.sir.erh.required.facade")
public class ErhApplication {
    public static ConfigurableApplicationContext ctx;

    public static void main(String[] args) {
        ctx=SpringApplication.run(ErhApplication.class, args);
    }
    ...
    private void createMessage(){
         String idEmeteur = "idEmeteur";
         String idDestinataire = "idDestinataire";
        String message = "message";
        String dateEnvoi = "dateEnvoi";
         String replied = "replied";
        for (int i = 1; i < 100; i++) {
            Message item = new Message();
            item.setIdEmeteur(fakeLong(idEmeteur, i));
            item.setIdDestinataire(fakeLong(idDestinataire, i));
            item.setMessage(fakeString(message, i));
            item.setDateEnvoi(fakeLocalDateTime(dateEnvoi, i));
    ...
    @Autowired
    MessageAdminService messageService;
    @Autowired
